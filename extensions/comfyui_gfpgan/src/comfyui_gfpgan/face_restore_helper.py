...
class FaceRestoreHelper(object):
    ...
    def __init__(self,
        ...
        self.face_parse = init_parsing_model(device=self.device, model_rootpath=model_rootpath)
    ...
    def get_face_landmarks_5(self,
        ...
        if max_faces is not None and len(self.det_faces) > max_faces:
            h, w, _ = self.input_img.shape
            face_areas = []
            for det_face in self.det_faces:
                face_area = (det_face[2] - det_face[0]) * (det_face[3] - det_face[1])
                face_areas.append(face_area)
            
            sorted_indices = np.argsort(face_areas)[::-1]
            top_indices = sorted_indices[:max_faces]
            
            self.det_faces = [self.det_faces[i] for i in top_indices]
            self.all_landmarks_5 = [self.all_landmarks_5[i] for i in top_indices]
    ...
    def add_soft_mask(self, mask):
      self.soft_masks.append(mask)
    ...
    def paste_faces_to_input_image(self, save_path=None, upsample_img=None):
        ...
        for restored_face, inverse_affine in zip(self.restored_faces, self.inverse_affine_matrices):
            ...
            if self.use_parse:
                ...
                MASK_COLORMAP = [0, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 0, 0, 0, 0, 0]
            ...
            self.add_soft_mask(inv_soft_mask)
