...
# Import utility functions from your musetalk package
from musetalk.utils.utils import get_file_type, get_video_fps, datagen, load_all_model
from musetalk.utils.preprocessing import get_landmark_and_bbox, read_imgs, coord_placeholder
from musetalk.utils.blending import get_image, get_image_prepare_material, get_image_blending

import scripts.chat_realtime as chat_realtime
from scripts.chat_realtime import generated_frame_queue, first_frame_event, turn_sequence, END_OF_VIDEO
...
def json_output_wrapper(data):
    return f'<json_data_output_wrapper>{json.dumps(data)}</json_data_output_wrapper>'
...
class Avatar:
    ...
    def process_frames(self, res_frame_queue, video_len, end_idx, frame_duration, current_turn_sequence):
        ...
        while self.idx < end_idx:
            ...
            #logging.info(f"current_turn_sequence {current_turn_sequence}; turn_sequence {turn_sequence.value}")
    ...
    def inference(self, audio_data, fps, current_turn_sequence, total_time):
        if audio_data is None:
            ...
            asyncio.run_coroutine_threadsafe(generated_frame_queue.put(SimpleNamespace(frame=END_OF_VIDEO, duration=0)), running_loop)
            return
        else:
        ...
        #logging.info("Processing audio took %.2fms", (time.time() - start_time) * 1000)
        video_num = len(whisper_chunks)
        frame_duration = total_time / video_num
        res_frame_queue = queue.Queue()
        end_idx = self.idx + video_num
        process_thread = threading.Thread(target=self.process_frames, args=(res_frame_queue, video_num, end_idx, frame_duration, current_turn_sequence))
...
# Define a global lock for inference, make sure inference one by one but it can support multiple threads process
...
async def start_live_inference(video_fps=25, audio_buffer_time=3):
    global running_loop
    #logging.info("Starting live inference mode…")
    running_loop = asyncio.get_running_loop()
    total_time = 0
    while True:
        # Buffer at least 3 seconds of audio (3s × 24000 samples/s × 2 bytes/sample)
        ...
        while len(buffered_audio) < required_bytes:
            audio_chunk = await chat_realtime.get_audio()
            if audio_chunk is None:
                send_end_tensor = True
                break
            buffered_audio.extend(audio_chunk)
            #logging.info("get_audio audio length: %d, Buffered: %d", len(audio_chunk), len(buffered_audio))
        #logging.info("Buffered audio length: %d", len(buffered_audio))
        if len(buffered_audio) == 0:
            if send_end_tensor:
                #logging.info("Buffering audio but no more data")
                await asyncio.to_thread(safe_inference, None, video_fps, turn_sequence.value, 0)
                send_end_tensor = False
            continue
        duration = len(buffered_audio)/2/24000
        total_time += duration
        #logging.info(f'Audio for processing time: {duration}s')
        ...
        # Convert to float and normalize (since 16-bit PCM typically ranges from -32768 to 32767)
