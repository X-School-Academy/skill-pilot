...
- `core/bin/skill-install` or `--all`: reconcile every discovered skill against `config/disabled_skills.json5`.
- `core/bin/skill-install --add <skill-name>`: refresh just the updated skill.
- `core/bin/skill-install --skills s1,s2,...`: install exactly the listed skills and remove any installed symlink not in the list.
