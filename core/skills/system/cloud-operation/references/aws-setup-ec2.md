...
- Verify: use skill `keys-safe-guard` to confirm `AWS_ACCESS_KEY_ID` and `AWS_REGION` are available.
...
- **Region** (read from `.env` via skill `keys-safe-guard`)
...
Use skill `keys-safe-guard` to get `AWS_REGION`.
